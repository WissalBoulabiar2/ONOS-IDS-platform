...
function MessageBubble({ msg }: { msg: Message }) {
  ...
  const formatContent = (text: string) => {
    return text
      .split('\n')
      .map((line, i) => {
        if (line.startsWith('### ')) return <h3 key={i} className="text-white font-bold text-sm mt-3 mb-1">{line.slice(4)}</h3>;
        if (line.startsWith('## ')) return <h2 key={i} className="text-white font-bold mt-3 mb-1">{line.slice(3)}</h2>;
          ...
          return (
            <div key={i} className="flex gap-2 my-0.5">
              ...
              <span dangerouslySetInnerHTML={{
                __html: line.slice(2).replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/`(.*?)`/g, '<code class="bg-slate-700 px-1 rounded text-cyan-300">$1</code>')
              }} />
            </div>
          );
        }
        if (line.match(/^\d+\./)) {
          return (
            <div key={i} className="flex gap-2 my-0.5">
              <span className="text-cyan-400 shrink-0">{line.match(/^\d+/)?.[0]}.</span>
              <span dangerouslySetInnerHTML={{
                __html: line.replace(/^\d+\.\s*/, '').replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/`(.*?)`/g, '<code class="bg-slate-700 px-1 rounded text-cyan-300">$1</code>')
              }} />
            </div>
          );
        }
        ...
        return (
          <p key={i} className="my-0.5" dangerouslySetInnerHTML={{
            __html: line.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/`(.*?)`/g, '<code class="bg-slate-700 px-1 rounded text-cyan-300">$1</code>')
          }} />
        );
      });
  };
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`shrink-0 flex h-8 w-8 items-center justify-center rounded-xl ${
        isUser ? 'bg-cyan-700' : 'bg-slate-700'
      }`}>
        {isUser
      ...
      <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${
        isUser
          ? 'bg-cyan-700/30 text-white border border-cyan-700/50'
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-white">NetGuard SOC Assistant</h1>
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl bg-slate-800 px-3 py-2 text-sm text-slate-400 hover:text-red-400 hover:bg-red-900/20 transition-colors"
        >
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <h2 className="text-lg font-bold text-white mb-1">NetGuard SOC Assistant</h2>
            ...
            <div className="grid grid-cols-2 gap-3 w-full max-w-lg">
              {QUICK_PROMPTS.map((qp) => (
                <button
                  ...
                  className="flex items-center gap-3 rounded-2xl border border-slate-700 bg-slate-800/50 p-4 text-left text-sm text-slate-300 hover:border-cyan-500/50 hover:bg-slate-800 transition-all"
                >
                  <qp.icon className="h-5 w-5 text-cyan-400 shrink-0" />
                  <span>{qp.label}</span>
                </button>
              ))}
            </div>
      ...
      {messages.length > 0 && (
        <div className="flex gap-2 overflow-x-auto px-6 py-2 border-t border-slate-800/50">
          {QUICK_PROMPTS.map((qp) => (
            <button
              key={qp.label}
              onClick={() => sendMessage(qp.prompt)}
              disabled={sending}
              className="shrink-0 flex items-center gap-1.5 rounded-xl border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs text-slate-400 hover:text-white hover:border-slate-600 transition-colors disabled:opacity-50"
            >
      ...
      <div className="border-t border-slate-800 px-6 py-4">
        <div className="flex gap-3 items-end">
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none rounded-2xl border border-slate-700 bg-slate-800 px-4 py-3 text-sm text-white placeholder-slate-500 focus:border-cyan-500 focus:outline-none max-h-32"
          ...
          <button
            ...
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-cyan-700 text-white hover:bg-cyan-600 transition-colors disabled:opacity-50"
          >
            {sending
