...
export default function ConfigPage() {
  ...
  return (
    <div className="flex h-full">
      {/* ── Device Sidebar ── */}
      <div className="w-56 shrink-0 border-r border-slate-800 bg-slate-950 overflow-y-auto">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors ${
                selectedDevice?.id === device.id
                  ? 'bg-cyan-700/20 border border-cyan-700/50'
                  : 'hover:bg-slate-800 border border-transparent'
              }`}
            >
              <div className={`h-8 w-8 shrink-0 flex items-center justify-center rounded-lg ${
                device.status === 'active' ? 'bg-cyan-700/30' : 'bg-slate-700'
              }`}>
        ...
        {!selectedDevice ? (
          <div className="flex h-full items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-slate-400">Select a device to configure</p>
            </div>
          </div>
        ) : (
          <div className="p-6">
            ...
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h1 className="text-xl font-bold text-white">{selectedDevice.name}</h1>
              ...
              <>
                ...
                {activeTab === 'ports' && (
                  <div className="space-y-3">
                    ...
                    {ports.length === 0 ? (
                      ...
                      <div key={port.port} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
                ...
                {activeTab === 'vlans' && (
                  <div>
                    <div className="mb-4 flex items-center justify-between">
                      ...
                      {canEdit && (
                        <button
                          ...
                          className="flex items-center gap-2 rounded-xl bg-cyan-700 px-4 py-2 text-sm text-white hover:bg-cyan-600"
                        >
                    ...
                    {showVLANForm && (
                      <div className="mb-4 rounded-2xl border border-cyan-700/30 bg-cyan-700/5 p-5">
                        <h3 className="mb-4 font-semibold text-white">Configure New VLAN</h3>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="mb-1 block text-xs text-slate-400">VLAN ID *</label>
                            <input
                              type="number"
                          ...
                          <div>
                            <label className="mb-1 block text-xs text-slate-400">Ports (comma separated)</label>
                            <input
                              type="text"
                        ...
                        <div className="mt-4 flex gap-3">
                          ...
                          <button
                            ...
                            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-cyan-700 px-4 py-2 text-sm text-white hover:bg-cyan-600 disabled:opacity-50"
                          >
                            {savingVLAN
                              ? <RefreshCw className="h-4 w-4 animate-spin" />
                    ...
                    {vlans.length === 0 ? (
                      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-8 text-center text-slate-500">
                        ...
                        {canEdit && (
                          <button
                            ...
                            className="mt-3 rounded-xl bg-cyan-700/20 px-4 py-2 text-sm text-cyan-400 hover:bg-cyan-700/30"
                          >
                            Add first VLAN
                          </button>
                        )}
                      ...
                      <div className="space-y-3">
                        {vlans.map(vlan => (
                          <div key={vlan.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
                            <div className="flex items-start justify-between">
                              <div>
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="font-medium text-white">{vlan.name}</span>
                                  <span className="rounded-lg bg-slate-700 px-2 py-0.5 text-xs text-slate-400 capitalize">
                                    {vlan.mode}
                                  </span>
                                </div>
                                <div className="mt-2 flex items-center gap-2 flex-wrap">
                                  {vlan.ports.map(p => (
                                    <span key={p} className="rounded-md bg-slate-800 px-2 py-0.5 text-xs text-slate-300">
                                      port {p}
                                    </span>
                                  ))}
                                </div>
                                {vlan.notes && (
                                  <p className="mt-1 text-xs text-slate-500">{vlan.notes}</p>
                                )}
                              </div>
                  ...
                  <div>
                    <div className="mb-4 flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 rounded-xl bg-slate-800 px-3 py-1.5 text-xs text-slate-300 hover:bg-slate-700"
                      >
                    ...
                    {hosts.length === 0 ? (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${
                              host.suspended ? 'bg-red-500/20' : 'bg-slate-700'
                            }`}>
                              <Monitor className={`h-5 w-5 ${
                                host.suspended ? 'text-red-400' : 'text-slate-300'
                              }`} />
                            </div>
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                {host.ip_addresses.slice(1).map(ip => (
                                  <span key={ip} className="text-xs text-slate-400">{ip}</span>
                                ))}
                                {host.suspended ? (
                        ...
                        <div className="space-y-3">
                          {history.map(entry => (
                            <div key={entry.id} className="relative flex gap-4">
                              <div className={`relative z-10 mt-2 h-3 w-3 shrink-0 rounded-full border-2 border-slate-900 ${
