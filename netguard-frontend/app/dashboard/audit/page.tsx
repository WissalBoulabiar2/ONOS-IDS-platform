...
const ACTION_STYLES: Record<string, string> = {
  ...
  mfa_success:    'bg-cyan-500/20 text-cyan-400',
  ...
  return (
    <div className="p-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-red-700/80 px-3 py-2 text-sm text-white hover:bg-red-600 transition-colors disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-blue-700/80 px-3 py-2 text-sm text-white hover:bg-blue-600 transition-colors"
            >
            ...
            <button
              onClick={downloadCEF}
              className="flex items-center gap-2 rounded-xl bg-purple-700/80 px-3 py-2 text-sm text-white hover:bg-purple-600 transition-colors"
            >
              <Download className="h-4 w-4" />
              CEF
            </button>
      ...
      {stats && (
        <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            ...
            <p className="text-xs text-slate-500">Last {stats.period_days} days</p>
          </div>
          <div className="rounded-2xl border border-green-500/20 bg-green-500/5 p-4">
            ...
            <p className="mt-2 text-2xl font-bold text-white">{stats.success_rate}%</p>
          </div>
          <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4">
            ...
            <p className="mt-2 text-2xl font-bold text-white">{stats.failed_events}</p>
          </div>
          <div className="rounded-2xl border border-orange-500/20 bg-orange-500/5 p-4">
            ...
            <p className="mt-2 text-2xl font-bold text-white">{stats.login_failures}</p>
          </div>
        </div>
      )}
      ...
      {stats && (
        <div className="mb-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <h3 className="mb-3 text-sm font-semibold text-white">Events by Action</h3>
            <div className="space-y-2">
              {stats.by_action.map(item => {
                const pct = Math.round((item.count / stats.total_events) * 100);
                return (
                  <div key={item.action} className="flex items-center gap-3">
                    <span className="w-28 text-xs text-slate-400 truncate">{item.action}</span>
                    ...
                    <span className="w-8 text-right text-xs text-slate-400">{item.count}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="space-y-2">
              {stats.by_user.map(item => (
                <div key={item.username} className="flex items-center justify-between">
                  <span className="text-xs text-slate-300 truncate max-w-[150px]">{item.username}</span>
            ...
            <div className="space-y-2">
              {stats.top_ips.map(item => (
                <div key={item.ip} className="flex items-center justify-between">
                  <span className="font-mono text-xs text-slate-300">{item.ip}</span>
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <span className="ml-auto text-sm text-slate-400">{logs.length} events</span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-800/50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs text-slate-400">User</th>
                <th className="px-4 py-3 text-left text-xs text-slate-400">Action</th>
                <th className="px-4 py-3 text-left text-xs text-slate-400">IP Address</th>
                <th className="px-4 py-3 text-left text-xs text-slate-400">Result</th>
              </tr>
            </thead>
                ...
                <tr key={log.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-lg px-2 py-0.5 text-xs ${
                      ACTION_STYLES[log.action] || 'bg-slate-700 text-slate-300'
                    }`}>
                      {log.action}
                    </span>
                  </td>
