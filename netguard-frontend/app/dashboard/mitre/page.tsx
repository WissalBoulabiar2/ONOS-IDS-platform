...
function getHeatColor(hits: number, max: number): string {
  if (hits === 0) return 'bg-slate-800/50 border-slate-700/50 text-slate-600';
  const intensity = hits / Math.max(max, 1);
  if (intensity >= 0.75) return 'bg-red-500/80 border-red-400 text-white font-bold';
  if (intensity >= 0.5) return 'bg-orange-500/70 border-orange-400 text-white';
  if (intensity >= 0.25) return 'bg-yellow-500/60 border-yellow-400 text-white';
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">MITRE ATT&CK Navigator</h1>
      ...
      {matrix && (
        <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            ...
            <p className="mt-1 text-3xl font-bold text-white">{matrix.total_tactics_active}</p>
            <p className="text-xs text-slate-500">of {matrix.matrix.length} total</p>
          </div>
          <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4">
            <p className="text-xs text-red-400">Detected Techniques</p>
            <p className="mt-1 text-3xl font-bold text-white">{matrix.total_detected_techniques}</p>
          </div>
          <div className="rounded-2xl border border-orange-500/20 bg-orange-500/5 p-4">
            <p className="text-xs text-orange-400">Timeline Events</p>
            <p className="mt-1 text-3xl font-bold text-white">{timeline.length}</p>
          </div>
          <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-4">
            <p className="text-xs text-cyan-400">Period</p>
            <p className="mt-1 text-3xl font-bold text-white">{days}d</p>
          </div>
        </div>
      )}
      ...
      <div className="mb-4 flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="h-4 w-8 rounded bg-slate-800/50 border border-slate-700/50" />
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
                ...
                <div key={tactic.tactic} className="flex flex-col gap-1">
                  {tactic.techniques.map(tech => (
                    ...
                    >
                      ...
                      {tech.hits > 0 && (
                        <div className="absolute -top-1 -right-1 h-3 w-3 rounded-full bg-red-500 text-[8px] flex items-center justify-center text-white font-bold">
                          {tech.hits}
                        </div>
                      )}
                    </div>
          ...
          {/* Tooltip */}
          {tooltip && (
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 rounded-xl border border-slate-600 bg-slate-900 p-3 shadow-xl min-w-[200px]">
              <p className="text-xs text-slate-400">{tooltip.tactic}</p>
              <p className="font-bold text-white">{tooltip.tech.full}</p>
              {tooltip.tech.hits > 0 ? (
                <p className="mt-1 text-xs text-red-400">⚠ {tooltip.tech.hits} detection(s)</p>
              ) : (
                <p className="mt-1 text-xs text-slate-500">Not detected</p>
              )}
            </div>
          )}
        </div>
            ...
            <div className="relative">
              ...
              {timeline.map((event, i) => (
                <div key={event.id} className="relative flex gap-4 pb-4">
                  <div className={`relative z-10 mt-1.5 h-3 w-3 shrink-0 rounded-full border-2 border-slate-900 ${SEVERITY_COLORS[event.severity] || 'bg-slate-500'}`} />
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className={`rounded-lg px-2 py-0.5 text-xs text-white ${SEVERITY_COLORS[event.severity] || 'bg-slate-500'}`}>
                            {event.severity}
                          </span>
                        </div>
                        <p className="mt-1.5 font-medium text-white text-sm">{event.title}</p>
                        <p className="text-xs text-cyan-400 font-mono mt-0.5">{event.technique}</p>
                        {event.source_ip && (
                          <p className="text-xs text-slate-500 mt-1">src: {event.source_ip}</p>
                        )}
                      </div>
