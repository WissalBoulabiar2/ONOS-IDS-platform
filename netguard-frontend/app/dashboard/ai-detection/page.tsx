    ...
    <div className="p-8">
      ...
      {!idsOnline ? (
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center">
          ...
          <h2 className="text-lg font-semibold text-white">IDS Service Unreachable</h2>
          <p className="mt-2 text-sm text-slate-400">
            The ML IDS service at{' '}
            <code className="text-cyan-400">192.168.91.133:8000</code> is not responding.
          </p>
          <p className="mt-1 text-xs text-slate-500">
            Make sure the service is running on your Linux VM:
          </p>
          <code className="mt-2 block rounded-xl bg-slate-900 px-4 py-2 text-xs text-green-400">
          ...
          <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs text-slate-400">Attack Classes</p>
              </div>
              <p className="mt-2 text-xl font-bold text-white">
                {health?.n_classes || '—'}
              </p>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-800/50">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs text-slate-400">Risk Score</th>
                        <th className="px-4 py-3 text-left text-xs text-slate-400">Detections</th>
                        <th className="px-4 py-3 text-left text-xs text-slate-400">Level</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800">
                      {topRisk.map((ip, i) => (
                        <tr key={i} className="hover:bg-slate-800/30">
                          ...
                          <td className="px-4 py-3 text-slate-300">{ip.count}</td>
                          <td className="px-4 py-3">
                            <span className={`rounded-lg px-2 py-0.5 text-xs ${
