        ...
        <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-slate-400">IPs Analyzed</p>
            </div>
            <p className="mt-2 text-2xl font-bold text-white">{stats.total_ips_analyzed}</p>
          </div>
          <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4">
            ...
            <p className="mt-2 text-2xl font-bold text-white">{stats.malicious_ips}</p>
          </div>
          <div className="rounded-2xl border border-purple-500/20 bg-purple-500/5 p-4">
            ...
            <p className="mt-2 text-2xl font-bold text-white">{stats.tor_nodes}</p>
          </div>
          <div className="rounded-2xl border border-orange-500/20 bg-orange-500/5 p-4">
            ...
            <p className="mt-2 text-2xl font-bold text-white">{stats.high_risk_ips}</p>
          </div>
        </div>
      )}
      ...
      <div className="mb-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
        ...
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-cyan-500 focus:outline-none"
          />
          <button
            onClick={searchTI}
            disabled={searching || !searchIP.trim()}
            className="flex items-center gap-2 rounded-xl bg-cyan-700 px-5 py-2.5 text-sm text-white hover:bg-cyan-600 transition-colors disabled:opacity-50"
          >
            {searching
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  {searchResult.cached && (
                    <span className="rounded-lg bg-slate-700 px-2 py-0.5 text-xs text-slate-400">
                      cached
                    </span>
                  )}
                </div>
...
{(searchResult.sources || []).length > 0 && (
  <div className="mt-2 flex items-center gap-2">
    ...
    {(searchResult.sources || []).map(s => (
      <span key={s} className="rounded-lg bg-slate-700 px-2 py-0.5 text-xs text-slate-300">
        {s}
      </span>
    ))}
  </div>
          ...
          {maliciousIPs.length === 0 ? (
            ...
            <div className="space-y-2">
              {maliciousIPs.map(ip => (
                <div
                  key={ip.ip}
                  className="flex items-center justify-between rounded-xl bg-slate-800/50 px-3 py-2.5"
                >
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {ip.country_code && (
                        <span className="text-xs text-slate-500">{ip.country_code}</span>
                      )}
                    </div>
                    {ip.isp && (
                      <p className="text-xs text-slate-500 truncate max-w-[200px]">{ip.isp}</p>
                    )}
              ...
              {stats.top_countries.map((item, i) => {
                ...
                return (
                  <div key={item.country} className="flex items-center gap-3">
                    ...
                    <span className="w-8 text-xs font-bold text-white">{item.country}</span>
