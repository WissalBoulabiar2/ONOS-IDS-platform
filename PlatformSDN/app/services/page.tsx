...
import { Cable, Layers3, Plus, RefreshCw, Router, Trash2 } from 'lucide-react';
    ...
    <AuthenticatedShell contentClassName="max-w-7xl">
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="max-w-2xl text-sm text-slate-300 sm:text-base">
              Manage Layer 2 virtual private LAN services directly from the platform using ONOS VPLS
              REST APIs.
            </p>
          </div>

          <div className="rounded-3xl border border-white/10 bg-white/5 p-5 backdrop-blur lg:min-w-[320px]">
            <p className="mb-2 text-xs uppercase tracking-[0.25em] text-sky-100">
              Service inventory
            </p>
            <p className="text-2xl font-semibold">{services.length} service(s)</p>
            <p className="mt-2 text-sm text-slate-300">
              {totalInterfaces} interface(s) mapped across all VPLS services
            </p>
            <Button
      ...
      <section className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
        <MetricCard
          ...
          icon={<Layers3 className="h-5 w-5 text-sky-600 dark:text-sky-400" />}
        />
        <MetricCard
          title="Interfaces"
          value={totalInterfaces}
          icon={<Cable className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />}
        />
        <MetricCard
          title="Selected"
          value={activeService?.interfaces?.length || 0}
          icon={<Router className="h-5 w-5 text-violet-600 dark:text-violet-400" />}
        />
      </section>
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div className="space-y-3">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge variant="outline">
                              {service.interfaces?.length || 0} interface(s)
                            </Badge>
                          </div>
                                ...
                                >
                                  <div className="flex items-start justify-between gap-3">
                                    <div>
                                      ...
                                      <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                        VLAN {serviceInterface.vlan || 'N/A'} •{' '}
                                        {serviceInterface.mac || 'MAC N/A'}
                                      </p>
                                    </div>
                                    <Button
                                      size="sm"
                                      ...
                                      onClick={() =>
                                        handleDeleteInterface(service.name, serviceInterface.name)
                                      }
                                      disabled={submitting}
                                    >
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="Select or type a VPLS service name"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <Input
                  value={connectPoint}
                  onChange={(event) => setConnectPoint(event.target.value)}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  placeholder="10.0.1.1/24,10.0.1.2/24"
                />
              </div>
              <div>
