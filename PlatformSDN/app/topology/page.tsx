...
function enrichEdges(edges: TopologyEdge[], loads: LinkLoadResponse['links']) {
  ...
  loads.forEach((entry) => {
    ...
    lookup.set(key, {
      ...
      throughput: n(
        (entry.raw as Record<string, unknown>).throughput ??
          (entry.raw as Record<string, unknown>).rate ??
          (entry.raw as Record<string, unknown>).latest
      ),
    });
  });
...
export default function TopologyPage() {
  ...
  const focusedEdge =
    (selectedEdge ? visibleEdges.find((edge) => edge.id === selectedEdge) : null) ||
    (hoveredEdge ? visibleEdges.find((edge) => edge.id === hoveredEdge) : null) ||
    null;
  const deviceCandidates = useMemo(
    () => visibleNodes.filter((node) => node.type !== 'host'),
    [visibleNodes]
  );
  ...
  const hottestLinks = useMemo(
    () =>
      [...visibleEdges]
        .filter((edge) => edge.utilization !== null)
  ...
  const runPathAnalysis = async () => {
    ...
    try {
      ...
      setPathError(
        fetchError instanceof Error ? fetchError.message : 'Failed to compute ONOS path'
      );
      setPathResult(null);
    } finally {
    ...
    <AuthenticatedShell mainId="topology-container" contentClassName="max-w-7xl">
          ...
          <div className="max-w-3xl">
            <div className="mb-4 flex flex-wrap gap-2">
              ...
              <Badge className="border-white/15 bg-white/10 text-slate-200">
                {autoRefresh ? 'Refresh every 5s' : 'Manual refresh'}
              </Badge>
            </div>
            ...
            <p className="max-w-2xl text-sm text-slate-300 sm:text-base">
              This view now merges ONOS topology, live link telemetry, device ports and path
              analysis so the technician can inspect the fabric without switching tools.
            </p>
          </div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    void exportToPDF(
                      'topology-container',
                      `SDN-Topology-${new Date().toISOString().split('T')[0]}.pdf`
                    )
                  }
                >
        ...
        <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
          ...
          <CardContent className="space-y-5">
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.4fr)_220px_220px_220px_220px]">
              ...
              <Select
                value={layout}
                onValueChange={(value) => setLayout(value as TopologyLayoutMode)}
              >
                <SelectTrigger>
              ...
              <Select value={pathSource || undefined} onValueChange={setPathSource}>
                <SelectTrigger>
                  <SelectValue placeholder="Source device" />
                </SelectTrigger>
                <SelectContent>
                  {deviceCandidates.map((node) => (
                    <SelectItem key={`src-${node.id}`} value={node.id}>
                      {node.label}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <Select value={pathDestination || undefined} onValueChange={setPathDestination}>
                <SelectTrigger>
                  <SelectValue placeholder="Destination device" />
                </SelectTrigger>
                <SelectContent>
                  {deviceCandidates.map((node) => (
                    <SelectItem key={`dst-${node.id}`} value={node.id}>
                      {node.label}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              <Button
                ...
                disabled={
                  pathLoading || !pathSource || !pathDestination || pathSource === pathDestination
                }
              >
                <GitBranch className={`mr-2 h-4 w-4 ${pathLoading ? 'animate-spin' : ''}`} />
                Analyze path
              </Button>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-950">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Include ONOS hosts and access links.
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-950">
                <div>
                  <p className="text-sm font-semibold">Show link labels</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Display ports on the graph.
                  </p>
                </div>
        ...
        <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
          <CardContent className="pt-6">
            ...
            <p className="text-3xl font-bold">
              {visibleEdges.filter((edge) => (edge.utilization ?? 0) >= HOT).length}
            </p>
          </CardContent>
        </Card>
        <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
          <CardContent className="pt-6">
            ...
            <p className="text-3xl font-bold">
              {visibleNodes.filter((node) => node.type === 'host').length}
            </p>
          </CardContent>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 gap-8 xl:grid-cols-3">
        <div className="space-y-8 xl:col-span-2">
          ...
          <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
            <CardHeader>
              <CardTitle className="text-xl">Hottest Links</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {hottestLinks.length > 0 ? (
                hottestLinks.map((edge) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full rounded-2xl border border-gray-200 p-4 text-left transition hover:border-cyan-300 hover:bg-cyan-50 dark:border-gray-800 dark:hover:border-cyan-700 dark:hover:bg-cyan-950/40"
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <Badge className={colors.badge}>{fmtPct(edge.utilization ?? null)}</Badge>
                      </div>
                      <div className="mt-3 h-2 rounded-full bg-gray-200 dark:bg-gray-800">
                        <div
                          className={`h-2 rounded-full ${colors.bar}`}
                          style={{ width: `${Math.max(4, edge.utilization ?? 4)}%` }}
                        />
                      </div>
                    </button>
                  );
                ...
                <div className="rounded-2xl bg-gray-50 p-5 text-sm text-gray-500 dark:bg-gray-950 dark:text-gray-400">
                  No live link telemetry is available for the current filtered view.
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
            ...
            <CardContent>
              {selectedNodeDetails ? (
                <div className="space-y-4">
                  ...
                  {selectedNodeDetails.type !== 'host' && (
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        variant="outline"
                        onClick={() => setPathSource(selectedNodeDetails.id)}
                      >
                        Use as source
                      </Button>
                      <Button
                        variant="outline"
                        onClick={() => setPathDestination(selectedNodeDetails.id)}
                      >
                        Use as destination
                      </Button>
                    </div>
                  ...
                  {selectedNodeDetails.type !== 'host' && (
                    <div className="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950">
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-sm font-semibold">Live ports</p>
                      ...
                      {portsLoading ? (
                        ...
                        <div className="mt-3 space-y-2">
                          {ports
                            ...
                            .map((port) => (
                              <div
                                key={`${selectedNodeDetails.id}-${port.portNumber}`}
                                className="rounded-xl border border-gray-200 bg-white p-3 text-xs dark:border-gray-800 dark:bg-gray-900"
                              >
                                <div className="flex items-center justify-between gap-2">
                                  <span className="font-semibold">Port {port.portNumber}</span>
                                  <Badge variant="outline">{port.live ? 'live' : 'down'}</Badge>
                                </div>
                                <div className="mt-2 grid grid-cols-2 gap-2 text-gray-500 dark:text-gray-400">
                                  <span>RX {fmtBytes(port.rxBytes)}</span>
                                  <span>TX {fmtBytes(port.txBytes)}</span>
                                </div>
                              </div>
                            ))}
                        </div>
                ...
                <div className="rounded-2xl bg-gray-50 p-5 text-sm text-gray-500 dark:bg-gray-950 dark:text-gray-400">
                  Select a node from the map to inspect its inventory data and live ports.
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
            <CardHeader>
              <CardTitle className="text-xl">Link Insight</CardTitle>
            </CardHeader>
            <CardContent>
              {focusedEdge ? (
                (() => {
                  ...
                  return (
                    <div className="space-y-4">
                      <div className="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950">
                        <div className="flex items-center justify-between gap-3">
                          <p className="text-sm font-semibold">
                            {nodeMap.get(focusedEdge.source)?.label || focusedEdge.source} {'->'}{' '}
                            {nodeMap.get(focusedEdge.target)?.label || focusedEdge.target}
                          </p>
                          <Badge className={colors.badge}>
                            {fmtPct(focusedEdge.utilization ?? null)}
                          </Badge>
                        </div>
                        <div className="mt-3 h-2 rounded-full bg-gray-200 dark:bg-gray-800">
                          <div
                            className={`h-2 rounded-full ${colors.bar}`}
                            style={{ width: `${Math.max(4, focusedEdge.utilization ?? 4)}%` }}
                          />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950">
                          ...
                          <p className="mt-2 text-sm font-semibold">
                            {focusedEdge.sourcePort || '?'} {'->'} {focusedEdge.targetPort || '?'}
                          </p>
                        </div>
                        <div className="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950">
                          <p className="text-xs uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">
                ...
                <div className="rounded-2xl bg-gray-50 p-5 text-sm text-gray-500 dark:bg-gray-950 dark:text-gray-400">
                  Hover or click a link to inspect utilization and ports.
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
            <CardHeader>
              <CardTitle className="text-xl">Path Analysis</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {pathError && (
              ...
              {activePath ? (
                <div className="rounded-2xl bg-gray-50 p-4 dark:bg-gray-950">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <Badge variant="outline">{activePath.summary}</Badge>
                  </div>
                  <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
                ...
                <div className="rounded-2xl bg-gray-50 p-5 text-sm text-gray-500 dark:bg-gray-950 dark:text-gray-400">
                  Choose two infrastructure devices, then run ONOS path analysis to highlight the
                  computed route directly on the map.
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <span>Green, amber and red links reflect ONOS load telemetry</span>
              </div>
            </CardContent>
          </Card>
        </div>
